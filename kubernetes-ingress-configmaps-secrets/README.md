...
PY
kubectl --context=devops-coursework -n devops-homework apply -f configmap.yaml -f backend-code.yaml
kubectl --context=devops-coursework -n devops-homework create secret generic demo-credentials --from-env-file=.env.secret
kubectl --context=devops-coursework -n devops-homework describe secret demo-credentials
```
...
```bash
kubectl --context=devops-coursework -n devops-homework apply -f frontend.yaml -f backend.yaml
kubectl --context=devops-coursework -n devops-homework rollout status deployment/demo-frontend --timeout=120s
kubectl --context=devops-coursework -n devops-homework rollout status deployment/demo-backend --timeout=120s
kubectl --context=devops-coursework -n devops-homework get pods,services
kubectl --context=devops-coursework -n devops-homework exec deployment/demo-backend -- python3 -c 'import os; print("environment:", os.environ["ENVIRONMENT"]); print("secret_loaded:", bool(os.environ.get("DEMO_TOKEN")))'
```

First check the API directly with a port-forward in one terminal:

```bash
kubectl --context=devops-coursework -n devops-homework port-forward service/demo-backend 8083:80
```
...
The instructor's manifests use `ingressClassName: nginx`. **The community Ingress NGINX project
retired in March 2026.** These files retain that class for compatibility with the disposable
classroom lab, not as a recommendation for a new production installation.
See the [official retirement notice](https://kubernetes.io/blog/2025/11/11/ingress-nginx-retirement/).

The classroom Minikube setup uses the commands below. If the instructor supplies a maintained
controller instead, set `ingressClassName` in both Ingress files to that controller's class and
use its actual namespace and Service for the port-forward.

```bash
minikube -p devops-coursework addons enable ingress
kubectl --context=devops-coursework -n ingress-nginx rollout status deployment/ingress-nginx-controller --timeout=180s
kubectl --context=devops-coursework get ingressclass
kubectl --context=devops-coursework -n devops-homework apply -f ingress.yaml
kubectl --context=devops-coursework -n devops-homework describe ingress demo-ingress
kubectl --context=devops-coursework -n ingress-nginx port-forward service/ingress-nginx-controller 8084:80
```
...
```bash
kubectl --context=devops-coursework -n devops-homework patch configmap demo-config --type=merge -p '{"data":{"ENVIRONMENT":"staging"}}'
kubectl --context=devops-coursework -n devops-homework exec deployment/demo-backend -- python3 -c 'import os; print(os.environ["ENVIRONMENT"])'
kubectl --context=devops-coursework -n devops-homework rollout restart deployment/demo-backend
kubectl --context=devops-coursework -n devops-homework rollout status deployment/demo-backend --timeout=120s
curl --fail -H 'Host: devops.test' http://localhost:8084/api/
```
...
```bash
kubectl --context=devops-coursework -n devops-homework apply -f configmap.yaml
kubectl --context=devops-coursework -n devops-homework rollout restart deployment/demo-backend
kubectl --context=devops-coursework -n devops-homework rollout status deployment/demo-backend --timeout=120s
```
...
kubectl --context=devops-coursework -n devops-homework create secret tls demo-tls --cert=tls.crt --key=tls.key
kubectl --context=devops-coursework -n devops-homework apply -f optional/ingress-tls.yaml
kubectl --context=devops-coursework -n ingress-nginx port-forward service/ingress-nginx-controller 8443:443
```
...
```bash
kubectl --context=devops-coursework -n devops-homework delete -f optional/ingress-tls.yaml -f ingress.yaml -f frontend.yaml -f backend.yaml -f backend-code.yaml -f configmap.yaml --ignore-not-found
kubectl --context=devops-coursework -n devops-homework delete secret demo-credentials demo-tls --ignore-not-found
rm -f .env.secret tls.key tls.crt
```

Stop port-forwards with Ctrl+C. Remove the Minikube profile only after finishing the other labs.
