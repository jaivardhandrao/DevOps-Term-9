...
print(
    f"PASS: {len(entries)} Kubernetes objects; namespaces, workload selectors, Service target ports, Ingress backends, ConfigMap references, external Secret contract, StatefulSet service, rollout selectors"
)
...
for p in [
    root / "README.md",
    root / "SUBMISSION.md",
    root / "KUBERNETES-VALIDATION.md",
    *root.glob("kubernetes-*/README.md"),
]:
    if not p.exists():
        continue
